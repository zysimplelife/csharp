...
{
    class WinInputHelpers
    {
        #region 从PInvoke.net 上粘贴过来的代码
        ...
        public static void ActivateWindow(string nClass, string nTitle)
        {
            IntPtr hwnd = FindWindow(nClass, nTitle);
            if (hwnd != IntPtr.Zero)
            {
        ...
        public static void winClickLeftMouse(string nclass, string ntitle, int dx, int dy)
        {
            IntPtr hwnd = FindWindow(nclass, ntitle);
            RECT rect = new RECT();
            if (hwnd != IntPtr.Zero)
